...
# Project Manager v2.5 - 项目上下文管理系统

> **隐私说明**：本技能仅在用户明确请求时操作文件（创建/更新 STATUS.md）。不会在后台持续监听对话，不会读取与项目无关的个人数据，不会自动发送数据到外部服务。所有数据本地存储于 workspace/projects/ 目录。
...
| **按需状态同步** | `memory/pm-checkpoint.json` | 用户启用后，检查新内容 |
...
4. **session 内状态同步**（按需启用）：用户主动启用后，定期检查新内容。
5. **分级管理**：轻重有别，杀鸡不用牛刀。
6. **意图优先**：仅在上下文明确指向项目管理意图时响应，不推断日常对话。
...
**仅在用户明确提及项目管理操作时响应。** 本技能不会对日常对话中的普通用语（如"继续"、"搞定了"、"记一下"）做出反应——只有当上下文明确指向项目操作时才介入。

#### 用户主动触发（按需响应）
- **新建**：新建项目、帮我建一个项目、创建项目
- **恢复**：回到XX项目、继续XX项目
- **保存**：保存项目状态、暂停项目
- **查看**：项目列表、查看进度
- **切换**：切换到另一个项目
...
#### 上下文明确关联时
- 用户提到具体的项目名或明确指代之前的项目工作时，匹配 index.md
- 无法确定时展示最近项目列表让用户选择，不做自动推断

**不触发的场景**：
- 日常对话中说"继续"、"搞定了"、"记一下"等通用表达
- 不涉及项目管理意图的任何讨论
...
4. **注册 session 状态同步**（见第 8 节，需用户主动启用）
...
6. **注册 session 状态同步**（见第 8 节，需用户主动启用）
...
**当用户主动表达离开意图时**（用户说"我先走了"、"明天继续"、主动结束对话）：
...
### 8. 🔄 Session 内状态同步（用户按需启用）

### 这是什么？

用户可以在项目操作时**主动启用**此功能。启用后，skill 在当前 session 中按用户指定的间隔检查是否有新内容需要记录到项目状态中。

### 启用方式

此功能**不会自动启用**。用户需明确表达意图，例如"帮我留意一下项目变化"或"开启状态同步"，才会注册。
...
| 新话题 | 突然讨论完全不同的话题 | 扫描 index.md 判断是否建项目 |
...
### 触发时机
- **项目新建时**：自动初始化日志条目
- **每次更新 STATUS.md 时**：自动追加本次会话的关键内容
- **项目归档时**：打包整个项目的完整对话历史
...
**第四步：写入风险日志**
- 所有识别出的风险写入对应项目 STATUS.md 的 `⚠️ Risk Log` 章节
...
9. **Session 结束收尾**：检测到用户离开意图时，做状态落盘（第 6 节）。
10. **新项目必须确认**：创建新项目前必须向用户确认（第 2 节）。
11. **按需启用**：session 状态同步仅在用户明确要求时启用（第 8 节）。

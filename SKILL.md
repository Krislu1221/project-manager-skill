...
description: "项目上下文隔离与状态管理系统 v2.5。核心解决：对话中遗忘、跨天上下文丢失、新话题漏记录。新增：甘特图自动生成、项目风险自动识别、依赖关系管理。触发词（穷举）：新建项目、帮我建一个、开始做、记录一下、回到、继续、上次那个、先停一下、保存状态、项目列表、换个话题、搞定了、这个很重要、别忘了、记一下、甘特图、时间线、进度图、风险扫描、检查风险。"
---
...
4. **session 内状态同步**：skill 激活后自动注册定期状态同步，持续检查新对话。
5. **分级管理**：轻重有别，杀鸡不用牛刀。
...
当 project-manager skill 被激活（用户触发新建/恢复/保存等项目操作）时，**在当前 session 里注册状态同步任务**。之后每隔一段时间检查新对话，发现新话题就自动建项目或归档到已有项目。

### 为什么需要？

用户不会每次都主动说"记一下"。状态同步解决了"你不说就不记"的问题——一次激活，持续生效。
...
11. **激活即注册**：skill 被激活后必须注册 session 状态同步（第 8 节）。
